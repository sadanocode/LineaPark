...
log_file = 'LineaLiquidity logs.xlsx'  # Файл логов
...
liq_deposit_swith = 1  # Включен ли депозит в пул ликвидности
liq_withdraw_switch = 1  # Включен ли вывод из пула ликвидности

liq_volume = [28, 32]  # Диапазон эфира, который будем заливать в пул ликвидности в $
liq_digs = [5, 8]  # Количество знаков после запятой для числа эфира
try_delay = [10, 15]  # Перерыв между доп попытками
deposit_slippage = 0.020  # slippage для ликвидности (например: 0.025 = 2.5 %)
deposit_slippage_max = 0.039  # Максимальное значение slippage для депозита
withdraw_slippage = 0.015  # slippage для ликвидности (например: 0.025 = 2.5 %)
withdraw_slippage_max = 0.029  # Максимальное значение slippage для вывода
