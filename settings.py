...
borrow_mult = [0.87, 0.96]  # Процент от доступной суммы займа, на который занимаем токен
borrow_digs = [6, 7]  # количество знаков после запятой для округления суммы заема

wstETH_volume = [56, 68]  # Стоимость эфира $, которое свапаем на wstETH для залога
