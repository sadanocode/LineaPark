...
gas_price_limit = 35  # Лимит цены газа в Эфире
...
#       / Параметры модулей /
eth_swap_switch = 1  # Включен ли свап ETH на USDC перед операциями
usdc_swap_switch = 1  # Включен ли свап USDC на ETH после операций
fwdx_swith = 1  # Включен ли депозит FWDX
zkdx_switch = 1  # Включен ли свап USDC на ZUSD и обратно в ZKDX

#       / Параметры операций /
usdc_volume = [20, 25]  # Количество USDC на которое будем свапать эфир для дальнейших операций
eth_volume_digs = [4, 6]  # Количество знаков после запятой для числа эфира, которое свапаем на USDC
slippage_USDC = 0.020  # slippage для свапов эфира на USDC и обратно

usdc_trunc = [0.80, 1.10]  # Сумма в USDC которая будет отрезаться от суммы депозита FWDX
weth_volume = [0.1, 0.2]  # Количество wETH, которое требуем взамен USDC для FWDX
weth_digs = [2, 3]  # Количество знаков после запятой для запрашиваемого количества wETH
